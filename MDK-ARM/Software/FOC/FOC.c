...
float electAngle;                        //电角度
float shaftAngle;                        //机械角度
float voltage_limit=12.6f;                //限幅电压值
float voltage_supply=12.6f;               //电压供应值
float duty_a,duty_b,duty_c;              //ABC占空比大小
float Ualpha,Ubeta;                      //反帕克变换参数
float Ua,Ub,Uc;                          //三相电压值
...
{
	//设置电压上限
	Ua  = Limit(Ua,0.0f,voltage_limit);
	Ub  = Limit(Ub,0.0f,voltage_limit);
	Uc  = Limit(Uc,0.0f,voltage_limit);
	
	//计算占空比
	duty_a = Limit(Ua/voltage_supply,0.0f,1.0f);   
	duty_b = Limit(Ub/voltage_supply,0.0f,1.0f);
	duty_c = Limit(Uc/voltage_supply,0.0f,1.0f);
	...
	//设置PWM值
	__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_1,ccr_a);   //Duty=set/8400
...
{
	elect_angle = ElectAngle_Limit(elect_angle+0);    //电角度加校正值
	elect_angle = elect_angle/57.32484f;
	//帕克逆变换
	Ualpha = Ud*cos(elect_angle)-Uq*sin(elect_angle);
	Ubeta  = Ud*sin(elect_angle)+Uq*cos(elect_angle);
	
	//克拉克逆变换
		Ua = Ualpha+voltage_supply/2.0f;
		Ub = (sqrt3*Ubeta-Ualpha)/2.0f+voltage_supply/2.0f;
		Uc = (-Ualpha-sqrt3*Ubeta)/2.0f+voltage_supply/2.0f;
...
void Clark_Park(AdcValue *adcvalue,Encoder_Struct *encoder_str,PID *pid)
{
	...
	*/
	Angle_rad = encoder_str->Return_Angle/57.32484f;
	if(Angle_rad>2*PI)
		Angle_rad -= 2*PI;
	else if(Angle_rad<0)
		Angle_rad += 2*PI;
	pid->Id_current = pid->Ialfa*arm_cos_f32(Angle_rad)+pid->Ibeta*arm_sin_f32(Angle_rad);
	pid->Iq_current = -pid->Ialfa*arm_sin_f32(Angle_rad)+pid->Ibeta*arm_cos_f32(Angle_rad);
}
...
void SVPWM(float Uq,float Ud,float Angle,SVPWM_Struct *svpwm_str)
{
	...
	*/
	...
	svpwm_str->PWMA = Limit(svpwm_str->PWMA,1,8250);
	svpwm_str->PWMB = Limit(svpwm_str->PWMB,1,8250);
	svpwm_str->PWMC = Limit(svpwm_str->PWMC,1,8250);
	__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_1,svpwm_str->PWMA);   //Duty=set/8400
