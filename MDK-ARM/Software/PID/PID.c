...
//	encoder_str->Encoder_raw_erro = (float)(encoder_str->Encoder_raw-encoder_str->Encoder_old_raw);

//	//16384/2，判断正反转过零点
//	if(encoder_str->Encoder_raw_erro>8192)          //反转过零点
//		encoder_str->Encoder_raw_erro = (float)(encoder_str->Encoder_raw-encoder_str->Encoder_old_raw)-16384;
//	else if(encoder_str->Encoder_raw_erro<-8192)    //正转过零点  
//		encoder_str->Encoder_raw_erro = 16384-(float)(encoder_str->Encoder_old_raw-encoder_str->Encoder_raw);

	
//	encoder_str->Encoder_raw_sum += encoder_str->Encoder_raw_erro;
//	
//	encoder_str->Encoder_old_raw = encoder_str->Encoder_raw;               //保留上一次编码器数值
//	encoder_str->Encoder_raw_erro_last = encoder_str->Encoder_raw_erro;
	
	if(pid->speed_count++>9)
	{
		pid->Speed_now = encoder_str->Encoder_raw_sum*3.662109375f;  //(Encoder_raw_sum/16384)*(1000*60) 为转/分
		pid->Speed_show = pid->Speed_show+alpha*((pid->Speed_now)-pid->Speed_show);
